...
config_transfer() {
    ...
    scp -i "${SSH_KEY_NAME}.pem" -o StrictHostKeyChecking=no config.sh server-setup.sh $SSH_USER@$PUBLIC_IP:~

    #Execute the script on EC2 instance
    echo "Executing the script on EC2 instance..."
    ssh -v -i "${SSH_KEY_NAME}.pem" -o StrictHostKeyChecking=no $SSH_USER@$PUBLIC_IP 'bash server-setup.sh'
